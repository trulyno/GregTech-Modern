package com.gregtechceu.gtceu.api.pipenet.amhs.funnel;
...
public class FunnelBehavior implements IManaged {
    ...
    public WidgetGroup createUI(Player entityPlayer) {
        ...
        for (int i = 0; i < slotSize; i++) {
            int finalI = i;
            var filterSlot = new SlotWidget(filterContainer, i, i * 18, 0).setBackgroundTexture(new GuiTextureGroup(GuiTextures.SLOT, GuiTextures.FILTER_SLOT_OVERLAY));
            var cacheSlot = new SlotWidget(cacheContainer, i, i * 18, 18).setBackgroundTexture(GuiTextures.SLOT);
            var ioButton = new EnumSelectorWidget<>(i * 18, 18 * 2, 18, 18, new IO[]{IO.NONE, IO.IN, IO.BOTH}, ios[finalI], io -> ios[finalI] = io);
